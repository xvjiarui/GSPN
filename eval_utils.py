...
def eval_recall_iou_nosem_fusion(gt_dir, pred_dir, iou_threshold=0.5, plot_dir=None):
    """ Input:  gt_dir contains test-xx.h5
    ...
    for item in test_h5_list:
        ...
        pred_mask1, pred_valid1, pred_conf1 = load_pred_h5_nosem(os.path.join(pred_dir, 'Level-1', item))
        pred_mask2, pred_valid2, pred_conf2 = load_pred_h5_nosem(os.path.join(pred_dir, 'Level-2', item))
        pred_mask3, pred_valid3, pred_conf3 = load_pred_h5_nosem(os.path.join(pred_dir, 'Level-3', item))
    ...
    for i in range(10):
        p = xx[i]
        recall_iou_list.append(np.sum(iou_arr >= p) / float(iou_arr.shape[0]))
...
def eval_recall_iou_nosem(stat_fn, gt_dir, pred_dir, iou_threshold=0.5, plot_dir=None):
...
def eval_hier_part_mean_iou(gt_labels, pred_labels, tree_constraint, tree_parents):
    ...
    assert tree_constraint.shape[0] == tree_parents.shape[
        0], 'ERROR: tree_constraint and tree_parents have different num_constraint'
